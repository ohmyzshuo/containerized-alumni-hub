...
// ExportWorksHandler is a handler function for exporting works' information by convocation year
func (h *Handler) ExportWorksHandler(c *gin.Context) {
	...
	file, err := h.svc.ExportWorksByYear(convocationYearInt)
	if err != nil {
	...
	fileName := fmt.Sprintf("works_report_%d.xlsx", convocationYearInt)
